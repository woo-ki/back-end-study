...
public class Ex01_servlet extends HttpServlet {
	 ...
	 * 4. doGet() 메소드
	 * 		1) GET방식의 요청인 경우에 자동으로 호출되는 메소드이다 (service() 메소드가 없으면)
	 * 		2) GET 방식의 요청 방법
	 * 			(1) <form method="get">
	 * 			(2) $.ajax({type: 'get'});
	 */
	 ...
	 * 5. doPost() 메소드
	 * 		1) POST방식의 요청인 경우에 자동으로 호출되는 메소드이다 (service() 메소드가 없으면)
	 * 		2) 요청(request)와 응답(response)를 모두 doGet() 메소드로 넘기고 자신은 아무 일도 하지 않는다.
	 */
