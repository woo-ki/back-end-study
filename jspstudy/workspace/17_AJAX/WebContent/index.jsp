...
<script type="text/javascript">
	...
	function fn_getString() {
		...
		$('#btn1').click(function() {
			...
			// $('#content'.html(str);	// 기존 요소를 지우고 추
		});
		*/
		$('#btn1').click(function() {
			$.ajax({
				url: '/17_AJAX/getString.black',	// 요청 URL
				type: 'get',	// 요청방식: get, post
				dataType: 'text',	// 받는(응답) 결과의 타입
				success: function(responseText) {	// responseText: BlackStringCommand의 반환값이 저장된다.
					$('#content').empty();
	...
	function fn_getJSON() {
		$('#btn2').click(function() {
			$.ajax({
				...
				type: 'get',
				dataType: 'json',	// 실제로는 JSON을 String으로 바꾼 결과를 받아온다.
