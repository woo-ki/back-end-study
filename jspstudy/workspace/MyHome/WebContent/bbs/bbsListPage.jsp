...
<script>
	function fn_bbsInsertPage(f) {
		f.action = '/MyHome/bbsInsertPage.bbs';
		f.submit();
	}
</script>
...
<form method="post">
	...
	<input type="button" value="새 글 작성하기" onclick="fn_bbsInsertPage(this.form)" /><br/><br/>
</form>

<table border="1">
	<caption>BBS 게시판</caption>
	<thead>
		<tr>
			<td>순번</td>
			<td>제목</td>
		...
		<tr>
			<td colspan="5">
				...
				<c:if test="${pageVo.beginPage <= pageVo.pagePerBlock}">
					<span><i class="far fa-caret-square-left"></i>&nbsp;</span>
				</c:if>
				<c:if test="${pageVo.beginPage > pageVo.pagePerBlock}">
					<a href="/MyHome/bbsListPage.bbs?page=${pageVo.beginPage - 1}"><i class="far fa-caret-square-left"></i>&nbsp;</a>
				</c:if>
				<%-- 2. 페이지 번호 --%>
				...
				<c:if test="${pageVo.endPage < pageVo.totalPage}">
					<a href="/MyHome/bbsListPage.bbs?page=${pageVo.endPage + 1}"><i class="far fa-caret-square-right"></i></a>
				</c:if>
			</td>
		</tr>
	</tfoot>
