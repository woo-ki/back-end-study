			...
			<c:if test="${not empty replyList}">
				<c:forEach var="replyDto" items="${replyList}">
					<tr>
						...
						<td><pre>${replyDto.rContent}</pre></td>
						<td>${replyDto.rDate}</td>
						<td><a href="/MyHome/replyDelete.reply?rNo=${replyDto.rNo}&page=${param.page}&bbs_no=${bbsDto.bbs_no}" onclick="return confirm('댓글을 삭제하시겠습니까?')"><i class="far fa-trash-alt"></i></a></td>
					</tr>
				</c:forEach>
			</c:if>
		</tbody>
