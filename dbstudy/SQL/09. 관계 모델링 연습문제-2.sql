...
CREATE TABLE 게시판(
    ...
    내용 VARCHAR2(4000),
    조회수 NUMBER NOT NULL,
    작성자 VARCHAR2(20) REFERENCES 회원(아이디),    -- 회원 테이블의 아이디를 참조하는 외래키
    작성일자 DATE NOT NULL
);
----------------------------------------------
CREATE TABLE 제조사(
...
CREATE TABLE 창고(
    ...
    사용유무 CHAR(1) NOT NULL,
    창고연락처 CHAR(15) NOT NULL
);
----------------------------------------------
CREATE TABLE 택배업체(
...
CREATE TABLE 주문(
    ...
    주문자 VARCHAR2(20) REFERENCES 회원(아이디),
    송장번호 NUMBER REFERENCES 배송(배송번호),
