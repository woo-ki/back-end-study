...
// 사용 중에는(메소드 실행) 가져가지 못하도록 synchronized 처리
// synchronized
...
class Cleaner {
	...
	public synchronized void livingRoomCleaning() throws Exception {
		...
		System.out.println("거실청소조금했음");
		notify();  // 거실청소 끝났다고 다른 스레드(papa)에게 알린다. 그럼 papa가 가져간다.
	...
	public synchronized void roomCleaning() throws Exception {
		...
		System.out.println("방청소조금했음");
		notify();  // 방청소 끝났다고 다른 스레드(mama)에게 알린다. 그럼 mama가 가져간다.
